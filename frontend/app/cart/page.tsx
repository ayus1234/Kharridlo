...
export default function CartPage() {
  ...
  const validateCart = async () => {
    ...
    try {
      const res = await fetch(`${apiBaseUrl}/api/v1/cart/${sessionId}/validate`, {
        method: "POST",
      });
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-6 border-b border-slate-200">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Authoritative commerce session with real-time inventory reservations and integer paise precision.
            </p>
          </div>
          ...
          {cart && (
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-xs font-medium">
                ...
                <span>30-min reservation hold active</span>
              </div>
              {cart.items.length > 0 && (
                <button
                  onClick={clearCart}
        ...
        {loading && (
          <div className="p-12 text-center">
            ...
            <p className="text-sm font-medium text-slate-600">Connecting to authoritative cart engine...</p>
          </div>
        )}
        ...
        {!loading && cart && cart.items.length === 0 && (
          <div className="text-center py-16 px-4 bg-white rounded-2xl border border-slate-200 shadow-sm max-w-md mx-auto my-8">
            ...
            <p className="text-xs text-slate-500 mt-1 mb-6">
              Browse the synthetic catalog and add products to test deterministic price calculations and inventory reservations.
            </p>
            <Link
              href="/catalog"
        ...
        {!loading && cart && cart.items.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {/* Order Summary & Validation Card (1 col) */}
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                ...
                <div className="mt-6 pt-4 border-t border-slate-100 space-y-2">
                  <button
                    ...
                    className="w-full py-2.5 rounded-xl text-xs font-semibold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 transition-colors flex items-center justify-center gap-1.5"
                  >
                    <ShieldCheck className={`w-4 h-4 ${actionLoading === "validate" ? "animate-spin" : ""}`} />
                    Verify Cart Fulfillability
                  </button>

                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/70 text-[11px] text-slate-500 leading-relaxed">
                    <span className="font-semibold text-slate-700 block mb-0.5">Policy Engine Boundary:</span>
                    Payment authorization and spending limit gates will be evaluated in Milestone 4.
                  </div>
                </div>
                  ...
                  <div className={`mt-4 p-3 rounded-xl border text-xs ${
                    ...
                    {validationResult.valid ? (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">All items verified in stock and fulfillable!</span>
                      </div>
                    ) : (
                      <div>
                        <span className="font-bold block mb-1">Validation Issues:</span>
