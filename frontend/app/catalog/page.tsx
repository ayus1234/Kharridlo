...
export default function CatalogPage() {
  ...
  const addToCart = async (product: MarketplaceProduct, e?: React.MouseEvent) => {
    ...
    // Commerce Authority Gate: unmapped external products cannot enter authoritative cart
    if (!product.mapping?.can_authoritative_checkout) {
      setSelectedProduct(product);
      return;
    }

    const internalId = product.mapping.internal_product_id || product.id;
    ...
    try {
      ...
      const res = await fetch(`${apiBaseUrl}/api/v1/cart/${sid}/items`, {
        ...
        body: JSON.stringify({ product_id: internalId, quantity: 1 }),
      });
      if (!res.ok) {
        const errJson = await res.json();
        throw new Error(errJson?.detail?.message || "Failed to add product to authoritative cart");
      }
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between gap-3">
              ...
              {selectedProduct.mapping?.can_authoritative_checkout ? (
                ...
                >
                  ...
                  <span>Add to Kharridlo Cart</span>
                </button>
              ) : (
                <Link
                  href="/assistant"
                  className="px-4 py-2 rounded-xl text-xs font-semibold text-indigo-700 bg-indigo-100 hover:bg-indigo-200 transition-colors inline-flex items-center gap-1.5"
                >
                  <span>Ask AI to Recommend Verified Alternative</span>
                </Link>
              )}
            </div>
          </div>
